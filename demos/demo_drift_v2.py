...
def print_drift_results(results, detector_type="DATA"):
    ...
    if 'features' in results:
        ...
        for feature, metrics in results['features'].items():
            ...
            print(f"   {feature:20s}: PSI={psi:.4f}, Change={mean_diff:+.1f}% {status_icon}")
    ...
    if 'distribution_comparison' in results:
        ...
        for cls, metrics in comp.items():
            ...
            print(f"   {cls:15s}: {ref_pct:5.1f}% → {curr_pct:5.1f}% ({diff_pct:+6.1f}%) {indicator}")
...
def demo_scenario_1_extreme_length():
    ...
    # Load data
    print("Loading data...")
    reference_data = pd.read_csv('data/processed/train.csv').sample(1000, random_state=42)
    ...
    drifted_data = simulator.simulate_extreme_length_drift(
        reference_data, 
        factor=3.0,  # Triple the length!
        proportion=1.0  # All samples
    )
    ...
    if results['overall_drift']:
        ...
        print(f"   System identified {results['features_with_drift']} features with significant drift")
    else:
...
def demo_scenario_2_heavy_slang():
    ...
    drifted_data = simulator.simulate_heavy_slang(
        reference_data,
        intensity=0.8  # 80% transformation rate
    )
...
def demo_scenario_3_population_collapse():
    ...
    print(f"Original Anxiety %: {(reference_data['label'] == 'Anxiety').mean()*100:.1f}%")
...
def demo_quick_verification():
    ...
    try:
        ...
        reference_data = pd.read_csv('data/processed/train.csv').sample(100, random_state=42)
        print("✓")
