...
class EnhancedDriftSimulator:
    ...
    def simulate_extreme_length_drift(
        self, data: pd.DataFrame, factor: float = 3.0, proportion: float = 1.0
    ) -> pd.DataFrame:
        """
        ...
        if factor > 1:
            ...
            for idx in indices:
                ...
                drifted.loc[idx, "text"] = repeated_text[
                    : int(len(original_text) * factor)
                ]
    ...
    def simulate_heavy_slang(
        self, data: pd.DataFrame, intensity: float = 0.8
    ) -> pd.DataFrame:
        """
        ...
        slang_dict = {
            ...
            ".": np.random.choice(
                [".", " fr.", " ngl.", " no cap.", " tbh."], p=[0.3, 0.2, 0.2, 0.2, 0.1]
            ),
        }
    ...
    def simulate_population_collapse(
        self,
        data: pd.DataFrame,
        ...
        if len(dominant_samples) < target_dominant:
            dominant_resampled = dominant_samples.sample(
                n=target_dominant, replace=True, random_state=42
            )
        else:
            dominant_resampled = dominant_samples.sample(
                n=target_dominant, replace=False, random_state=42
            )
        ...
        # Combine
        drifted = (
            pd.concat([dominant_resampled, other_resampled])
    ...
    def simulate_sentiment_flip(
        self, data: pd.DataFrame, proportion: float = 0.6
    ) -> pd.DataFrame:
        """
    ...
    def simulate_formality_shift(
        self, data: pd.DataFrame, direction: str = "informal", intensity: float = 0.7
    ) -> pd.DataFrame:
        """
    ...
    def simulate_heavy_noise(
        self, data: pd.DataFrame, noise_level: float = 0.3
    ) -> pd.DataFrame:
        """
    ...
    def simulate_catastrophic_drift(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        drifted = self.simulate_extreme_length_drift(
            drifted, factor=0.4, proportion=0.6
        )  # 60% shorter
        ...
        drifted = self.simulate_population_collapse(
            drifted, dominant_class="Anxiety", dominance=0.85
        )
    ...
    def generate_comparison_report(
        self, original: pd.DataFrame, drifted: pd.DataFrame
    ) -> Dict:
        """
        ...
        report["text_length"] = {
            ...
            "change_pct": float(
                ((drift_lengths.mean() - orig_lengths.mean()) / orig_lengths.mean())
                * 100
            ),
        ...
        report["word_count"] = {
            ...
            "change_pct": float(
                ((drift_words.mean() - orig_words.mean()) / orig_words.mean()) * 100
            ),
        }
    ...
    def print_report(self, report: Dict):
        ...
        print(
            f"   Original: {tl['original_mean']:.1f} ± {tl['original_std']:.1f} chars"
        )
        print(f"   Drifted:  {tl['drifted_mean']:.1f} ± {tl['drifted_std']:.1f} chars")
        print(
            f"   Change:   {tl['change_pct']:+.1f}% {'🚨' if abs(tl['change_pct']) > 20 else '✅'}"
        )
        ...
        print(
            f"   Change:   {wc['change_pct']:+.1f}% {'🚨' if abs(wc['change_pct']) > 20 else '✅'}"
        )
        ...
        if "class_distribution" in report:
            ...
            for cls, pct in sorted(report["class_distribution"]["drifted"].items()):
                ...
                indicator = (
                    "🚨" if abs(change) > 50 else ("⚠️" if abs(change) > 20 else "✅")
                )
