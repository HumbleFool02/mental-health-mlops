...
class TestAPIEndpoints:
    ...
    def test_predict_normal(self):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["prediction"] in ["Normal", "Anxiety", "Depression", 
                                       "Suicidal", "Bipolar", "Stress"]
    ...
    def test_predict_depression(self):
        ...
        data = response.json()
        assert "prediction" in data
        print(f"✅ Depression prediction: {data['prediction']} ({data['confidence']:.2%})")
    ...
    def test_drift_status(self):
        ...
        print(f"✅ Drift status: Score={data['drift_score']:.4f}, Predictions={data['predictions_since_check']}")
